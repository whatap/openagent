...
// SetConfigLogger sets the logger in the Config object
func SetConfigLogger(logger logger.Logger) {
	if confInstance != nil {
		confInstance.AppLogger = logger
	}
}

// GetConfigLogger gets the logger from the Config object
func GetConfigLogger() logger.Logger {
	if confInstance != nil {
		return confInstance.AppLogger
	}
	return &logger.EmptyLogger{}
}

// SetLogger sets the logger in the Config object (exported version)
func SetLogger(logger logger.Logger) {
	SetConfigLogger(logger)
}

// GetLogger gets the logger from the Config object (exported version)
...
func newFileConfig(opts ...FileConfigOption) *FileConfig {
	...
	// Set the application logger from the config or use an EmptyLogger if not provided
	if c.conf.appLogger != nil {
		c.AppLogger = c.conf.appLogger
	} else {
		c.AppLogger = &logger.EmptyLogger{}
	}
...
func (this *FileConfig) SetAppLogger(logger logger.Logger) {
	this.AppLogger = logger
}

// GetAppLogger returns the application logger
func (this *FileConfig) GetAppLogger() logger.Logger {
	return this.AppLogger
}

// SetLogger sets the application logger (exported version)
func (this *FileConfig) SetLogger(logger logger.Logger) {
	this.AppLogger = logger
}

// GetLogger returns the application logger (exported version)
func (this *FileConfig) GetLogger() logger.Logger {
	return this.AppLogger
}
