...
func (sm *ScraperManager) createScraperTaskFromTarget(target *discovery.Target) *ScraperTask {
	...
	// Get the original target type from metadata for proper logging
	originalType, _ := target.Metadata["type"].(string)
	if originalType == "" {
	...
	// Note: We use StaticEndpoints internally for efficiency, but preserve original type info for logging
