...
func NewParamSecurity(opts ...CryptoOption) *ParamSecurity {
	p := new(ParamSecurity)
...
func GetParamSecurity(opts ...CryptoOption) *ParamSecurity {
...
func (this *ParamSecurity) Reload() {
	...
	defer func() {
		if r := recover(); r != nil {
			conf.Log.Println("WA1000", " Recover ", r)
		}
		if f != nil {
			f.Close()
		}
	}()
	home := conf.WhatapHome
	...
	if os.IsNotExist(err) {
		...
		if err != nil {
			this.Log.Println("WA1001", " Create File Error", err)
			return
		}
		...
		if err != nil {
			this.Log.Println("WA1002", " Open File Error", err)
			return
		}
		...
		n, err := f.Read(b)
		if err != nil {
			this.Log.Println("WA1003", " Read File Error", err)
		} else {
			if n > 3 {
...
func (this *ParamSecurity) Encrypt(b []byte, crc *ref.BYTE) []byte {
	j := 0
	//this.Log.Println("Before Encrypt", b)
